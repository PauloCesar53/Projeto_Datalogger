...
# 2. Definir as colunas para os eixos do GRÁFICO 3D (Amostra, GyroX, GyroY)
x_gyro = df['amostra']
y_gyro = df['GyroX']
z_gyro = df['GyroY'] # GyroY será o eixo Z no gráfico 3D

# 3. Plotar o novo gráfico 3D
print("\n--- Gráfico 3D: Amostra vs GyroX vs GyroY ---")
fig_gyro = plt.figure(figsize=(10, 7)) # Cria uma nova figura
ax_gyro = fig_gyro.add_subplot(111, projection='3d') # Adiciona um subplot 3D

ax_gyro.plot(x_gyro, y_gyro, z_gyro, marker='o', linestyle='-', color='g') # Plota os dados com cor verde

# Configurar os rótulos dos eixos e o título
ax_gyro.set_xlabel('Amostra')
ax_gyro.set_ylabel('GyroX')
ax_gyro.set_zlabel('GyroY')
ax_gyro.set_title('Gráfico 3D: Amostra vs GyroX vs GyroY')
...
# Se você quiser também um gráfico 2D separado, Amostra vs GyroX:
print("\n--- Gráfico 2D: Amostra vs GyroX ---")
plt.figure(figsize=(8, 5))
plt.plot(x_gyro, y_gyro, 'go-') # 'go-' significa pontos verdes e linha
plt.title('Gráfico 2D: Amostra vs GyroX')
plt.xlabel('Amostra')
plt.ylabel('GyroX')
plt.grid(True)
plt.show()